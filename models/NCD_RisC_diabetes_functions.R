...
LogLik <- function(alpha, zeta) {
    p <- pnorm(alpha)                                       # Prevalence of total diabetes
    q <- pnorm(zeta) * p                                    # Prevalence of treated diabetes
    r <- (p - q) / (1 - q)                                  # Proportion of newly-detected diabetes among people who were not treated for diabetes

    lik1 <- dbinom(y_P, number, p[flag == 0], log = TRUE)   # Log likelihood of binomial process for number of individuals with diabetes given alpha when components are not available
    lik2 <- dbinom(y_Q, number_q, q[flag == 1], log = TRUE) # Log likelihood of binomial process for number of treated individuals given alpha and zeta when components are available
    lik3 <- dbinom(y_R, number_r, r[flag == 1], log = TRUE) # Log likelihood of binomial process for number of untreated individuals with diabetes given alpha and zeta when components are available

    res <- rep(NA, I)
    res[flag == 0] <- lik1           # Log likelihood of binomial process for number of individuals with diabetes given alpha when components are not available
    res[flag == 1] <- lik2 + lik3    # Log likelihood of binomial process for number of individuals with diabetes given alpha and zeta when components are available
